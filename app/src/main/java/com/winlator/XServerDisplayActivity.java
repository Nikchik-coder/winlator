...
public class XServerDisplayActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        xServer.setRelativeMouseMovement(preferences.getBoolean("relative_mouse_movement", false));
    ...
    private void exit() {
        ...
        Intent intent = getIntent();
        // Only return to the container file manager when the launch originated there.
        if (intent.getBooleanExtra("return_to_file_manager", false) && intent.hasExtra("exec_path")) {
    ...
    private void setupUI() {
        ...
        inputControlsView.setShowTouchscreenControls(preferences.getBoolean("show_touchscreen_controls", true));
        ...
        if (controlsProfile.isEmpty() && container != null) controlsProfile = container.getExtra("controlsProfile");
    ...
    private void showInputControlsDialog() {
        ...
        dialog.setOnConfirmCallback(() -> {
            boolean relativeMouse = cbRelativeMouseMovement.isChecked();
            boolean showTouchscreenControls = cbShowTouchscreenControls.isChecked();
            preferences.edit()
                .putBoolean("relative_mouse_movement", relativeMouse)
                .putBoolean("show_touchscreen_controls", showTouchscreenControls)
                .apply();

            xServer.setRelativeMouseMovement(relativeMouse);
            ...
            if (position > 0) {
                ...
                String selectedId = String.valueOf(selected.id);
                if (shortcut != null) {
                    shortcut.putExtra("controlsProfile", selectedId);
                    shortcut.saveData();
                }
                else if (container != null) {
                    container.putExtra("controlsProfile", selectedId);
                    container.saveData();
                }
            }
            else {
                hideInputControls();
                if (shortcut != null) {
                    shortcut.putExtra("controlsProfile", null);
                    shortcut.saveData();
                }
                else if (container != null) {
                    container.putExtra("controlsProfile", null);
                    container.saveData();
                }
            }
        });
        ...
        if (shortcut != null) {
            ...
            if (intent.hasExtra("exec_path")) {
                ...
                if (execPath == null || execPath.isEmpty()) {
                    // Fallback: Z: is typically mapped to filesystem root in Wine. This avoids crashes when
                    // the container doesn't have a drive mapping for the provided unix path yet.
